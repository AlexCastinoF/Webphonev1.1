...
const WebSoftphone: React.FC = () => {
  ...
  const [ringingTime, setRingingTime] = useState<string>('');
    ...
    try {
      ...
      inviter.stateChange.addListener((state: SessionState) => {
        ...
        if (state === SessionState.Establishing) {
          ...
          //Alex ringtone
          //useRingTone(callState.callStatus === 'Chamando...');
  ...
  const handleAcceptCall = async () => {
    ...
    try {
      setCallState(prev => ({ ...prev, callStatus: 'Aceitando chamada...' }));
    ...
    if (sessionRef.current?.state === SessionState.Established) {
      try {
        if (callState.isOnHold) {
          sessionRef.current.unhold().catch(error => {
            console.error('Error unholding call:', error);
          });
        } else {
          sessionRef.current.hold().catch(error => {
              ...
              <div className="flex gap-2 flex-shrink-0 flex-wrap justify-end">
                ...
                {callState.isInCall && (
                  <>
                    ...
                    <button 
                      onClick={handleHold} 
                      className={`p-2 rounded-full hover:bg-blue-50 ${callState.isOnHold ? 'text-red-600' : 'text-blue-600'}`}
                    >
